...
Avgoprofit <- subset(Avgoprofit, select = c(Order_Date, Profit, Customer_Segment, Product_Category, Region))

Avgoprofit$Order_Date <- as.yearmon(Avgoprofit$Order_Date, "%Y-%m")

Avgoprofit <- Avgoprofit %>% filter(Order_Date >= "jan 2017", Order_Date <= "des 2017")%>% group_by(Customer_Segment, Product_Category, Region) %>%
